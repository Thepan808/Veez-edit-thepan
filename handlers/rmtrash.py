...
async def clear_downloads(_, message: Message):
    ...
    if ls_dir:
        ...
        await message.reply_text("✅ **Removi todos os download files**")
    else:
        await message.reply_text("❌ **Não tem downloads files**")
...
async def clear_jpg_image(_, message: Message):
    ...
    if ls_dir:
        for dta in os.listdir(pth):
            os.system("rm -rf *.jpg")
        await message.reply_text("✅ **Apagado com sucesso**")
    else:
        await message.reply_text("✅ **já está limpo**")
